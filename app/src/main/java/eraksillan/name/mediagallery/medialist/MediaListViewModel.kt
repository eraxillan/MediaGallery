...
class MediaListViewModel @AssistedInject constructor(
    ...
    val mediaPagingVM = PagingViewModel<LocalMedia>(
    ...
    init {
        mediaPagingVM.getPageData()
    }

    fun navigateToDetail(data: LocalMedia) {
        navController.navigate(Route.MediaDetail(data))
    }
    ...
    fun onEvent(action: MediaListAction) {
        when (action) {
            ...
            is MediaListAction.SortTypeClicked -> {
                val listCopy = mediaPagingVM.list.toList()
                mediaPagingVM.reset()
                mediaPagingVM.listState = PagingListState.PAGINATION_EXHAUST
                when (action.index) {
                    // Sort by members count
                    0 -> mediaPagingVM.list.addAll(listCopy.sortedBy { it.members }.asReversed())
                    // Sort by start date
                    //1 -> mediaPagingVM.list.addAll(listCopy.sortedBy { it.aired.from?.epochSeconds })
                    // Sort by score
                    2 -> mediaPagingVM.list.addAll(listCopy.sortedBy { it.score }.asReversed())
                }
            }

            is MediaListAction.NavigateToDetail -> {
                navigateToDetail(action.data)
            }
        }
    }
