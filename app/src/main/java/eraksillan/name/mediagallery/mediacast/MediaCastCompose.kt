...
@Composable
fun MediaCastCompose(data: List<LocalMedia.Cast>, viewModel: MediaCastViewModel) {
    Scaffold(
        topBar = {
            CenterAlignedTopAppBar(
                ...
                navigationIcon = {
                    IconButton(onClick = { viewModel.onEvent(MediaCastAction.NavigateBackClicked) }) {
                        Icon(
...
@Composable
private fun MediaCastContentCompose(
    data: List<LocalMedia.Cast>,
    onEvent: (MediaCastAction) -> Unit,
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(16.dp)) {
                ...
                AsyncImage(
                    ...
                    modifier = Modifier
                        .width(64.dp)
                        .height(64.dp)
                        .clickable { onEvent(MediaCastAction.CharacterClicked(cast.character.malId)) },
                ...
                Column {
                    // Character name
                    Text(text = cast.character.name, fontWeight = FontWeight.Bold)
                    // Type of character
                    Text(text = cast.role)
                    // Japanese voice actor
                    Text(text = stringResource(R.string.voice, voiceActorName))
                }
            }
        }
    }
